...
def generate_images(network_pkl, seeds, truncation_psi):
    ...
    print('Done! Please check images in %s',dnnlib.make_run_dir_path(''))
...
def style_mixing_example(network_pkl, row_seeds, col_seeds, truncation_psi, col_styles, minibatch_size=4):
    ...
    all_seeds = list(set(row_seeds + col_seeds))#字符串row_seeds和col_seeds的字符交集在转换为列表
    all_z = np.stack([np.random.RandomState(seed).randn(*Gs.input_shape[1:]) for seed in all_seeds]) # [minibatch, component]
    # print("shape of all_z is:%",all_z.shape)(10,512)
    all_w = Gs.components.mapping.run(all_z, None) # [minibatch, layer, component]
    # print("shape of all_w is:%",all_w.shape)(10,18,512)
    all_w = w_avg + (all_w - w_avg) * truncation_psi # [minibatch, layer, component]
    print("shape of all_w is:%", all_w.shape)
    ...
    for row_seed in row_seeds:
        for col_seed in col_seeds:
            ...
            # print("shape of w is:%",w.shape)(18,512)
            w[col_styles] = w_dict[col_seed][col_styles]
            #print('the shape of w[col_styles] is')
